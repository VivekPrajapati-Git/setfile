...
def callback():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute("INSERT OR REPLACE INTO users (google_id, email, access_token , refresh_token , expires_at) VALUES (?, ?, ?, ?, ?)", (google_id, email, credentials.token, credentials.refresh_token, credentials.expiry))
    db.commit()
    db.close()

    path = pathlib.Path(__file__).resolve().parent.parent/'session.json'
    with open(path,'w') as f:
        f.write(f' id : {user_info['id']} , email : {user_info['email']}')
